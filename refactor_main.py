...
from model.refactored_model import EndToEndModel

def test_main():
    with open('vocab.pkl', 'rb') as f:
        vocab = pickle.load(f)
    model = EndToEndModel('vit', 'routing', len(vocab))
    pic = torch.randn(2,3,576,288)
    label = torch.randint(0, 100, (2, 225, 13))
    mask = torch.ones(2, 225).bool()
    pic = torch.randn(1,3,576,288)
    x = model(pic, label, mask=mask)
    y = model.generate(pic, vocab, 225)
