...
def main():
    ...
    opt = optim.AdamW(learner.parameters(), lr=1e-1)
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=len(train_dataloader) * 5, eta_min=1e-4)
    ...
    last_out = attns[0][-1]
    for hdx in range(len(last_out)):
        head_img = last_out[hdx][0][1:]
        ...
        save_image(im, 'attn_{}.png'.format(hdx))
